...
def main():
    ...
    if not os.path.exists(config.VILLAGES_WITH_POP_PATH):
        raise FileNotFoundError(
            f"{config.VILLAGES_WITH_POP_PATH} not found. Run 07_prepare_population.py first."
        )
    if not os.path.exists(config.RED_ZONES_FINAL_GEOJSON):
        raise FileNotFoundError(
            f"{config.RED_ZONES_FINAL_GEOJSON} not found. Run 06_generate_redzone_map.py first."
        )
    ...
    if villages.crs != zones.crs:
        print(
            f"CRS mismatch (villages={villages.crs}, zones={zones.crs}) — reprojecting zones to match villages"
        )
    ...
    pivot = overlay.pivot_table(
        index="_row_id",
        columns="zone_class",
        values="overlap_area_sqm",
        aggfunc="sum",
        fill_value=0,
    )
    pivot.columns = [
        f"area_zone_{int(c)}" for c in pivot.columns
    ...
    villages["pct_green"] = villages["area_zone_0"] / villages[
        "village_area_sqm"
    ].replace(0, pd.NA)
    villages["pct_yellow"] = villages["area_zone_1"] / villages[
        "village_area_sqm"
    ].replace(0, pd.NA)
    villages["pct_red"] = villages["area_zone_2"] / villages[
        "village_area_sqm"
    ].replace(0, pd.NA)
    villages[["pct_green", "pct_yellow", "pct_red"]] = villages[
        ["pct_green", "pct_yellow", "pct_red"]
    ].fillna(0.0)

    # Estimated affected population (population * fraction in red zone)
    villages["estimated_affected_population"] = (
        (villages["population"] * villages["pct_red"]).round().astype(int)
    )
    ...
    print(
        f"Total estimated affected population (IMMEDIATE + SHORT_TERM): "
        f"{at_risk['estimated_affected_population'].sum():,}"
    )

    print("\nTop 10 highest-priority villages by estimated affected population:")
    top10 = villages.sort_values("estimated_affected_population", ascending=False).head(
        10
    )
    print(
        top10[
            [
                config.VILLAGE_NAME_COLUMN,
                "population",
                "pct_red",
