...
def main():
    ...
    for path in [
        config.RED_ZONES_FINAL_GEOJSON,
        config.SLOPE_PATH,
    ...
    if use_real_landuse:
        print(
            "Land use data found — computing real developable-land fraction "
        ...
        with rasterio.open(
            config.DEVELOPABLE_MASK_PATH, "w", **profile_for_mask
        ) as dst:
            dst.write(developable_mask, 1)
        print(
            f"Developable mask saved: {config.DEVELOPABLE_MASK_PATH} "
            f"({developable_mask.sum()} developable pixels of {developable_mask.size})"
        )
    else:
        print(
            "No land use data found — falling back to flat DEVELOPABLE_LAND_FRACTION "
    ...
    green_sites = green_sites[
        green_sites["area_sqkm"] >= config.MIN_SITE_AREA_SQKM
    ].reset_index(drop=True)
    print(
        f"Sites after minimum-size filter ({config.MIN_SITE_AREA_SQKM} sqkm): {len(green_sites)}"
    )

    if len(green_sites) == 0:
        raise ValueError(
            "No candidate sites remain after filtering. Lower MIN_SITE_AREA_SQKM in config.py."
        )
    ...
    green_sites["avg_stream_distance_m"] = [
        s["mean"] if s["mean"] is not None else config.WATER_PROXIMITY_CAP_M
    ...
    # Estimated capacity
    green_sites["buildable_area_sqm"] = green_sites["buildable_pct"] * (
        green_sites["area_sqkm"] * 1_000_000
    )

    if use_real_landuse:
        developable_stats = zonal_stats(
            green_sites, config.DEVELOPABLE_MASK_PATH, stats=["mean"], nodata=255
        )
        green_sites["developable_pct"] = [
            s["mean"] if s["mean"] is not None else 0.0 for s in developable_stats
        ]
        green_sites["developable_area_sqm"] = green_sites["developable_pct"] * (
            green_sites["area_sqkm"] * 1_000_000
        )
        print(
        ...
        green_sites["developable_pct"] = (
            green_sites["buildable_pct"] * config.DEVELOPABLE_LAND_FRACTION
        )
        green_sites["developable_area_sqm"] = (
            green_sites["buildable_area_sqm"] * config.DEVELOPABLE_LAND_FRACTION
        )
    ...
    green_sites = green_sites.sort_values(
    ...
    if use_real_landuse:
        print("  (Using real land-use-derived developable fraction — see landuse.tif)")
    else:
        print(
            f"  (Using flat DEVELOPABLE_LAND_FRACTION={config.DEVELOPABLE_LAND_FRACTION} — see config.py)"
        )

    total_capacity = green_sites["estimated_population_capacity"].sum()
    if total_capacity > 2_000_000:
        print(
            "WARNING: total capacity exceeds Darjeeling's entire district population (~2M). "
    ...
    print(
        green_sites[
            [
                ...
                "area_sqkm",
                "buildable_pct",
                "avg_stream_distance_m",
                "suitability_score",
                "estimated_population_capacity",
            ]
        ].head(10)
    )
    ...
    print(
        "NOTE: suitability reflects terrain + water proximity only (no road data yet)."
    )
