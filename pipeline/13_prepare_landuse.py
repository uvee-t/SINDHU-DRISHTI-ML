...
def main():
    ...
    if not os.path.exists(config.DEM_CLIPPED_UTM):
        raise FileNotFoundError(f"{config.DEM_CLIPPED_UTM} not found. Run 01_prepare_dem.py first.")

    tile_paths = find_landuse_rasters()
    print(f"Found {len(tile_paths)} land use tile(s): {[os.path.basename(t) for t in tile_paths]}")
    ...
    subprocess.run([
        ...
        "-r", "near",  # categorical data — never bilinear
        "-overwrite",
